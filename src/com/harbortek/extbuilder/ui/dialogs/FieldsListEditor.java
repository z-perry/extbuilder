...
public class FieldsListEditor extends Composite {
	
	static String[] TYPES = new String[]{"","string","int","float","boolean","date"};
	...
	public FieldsListEditor(Composite parent, int style) {
		super(parent, style);
	...
	private void createControls() {
		...
		new TableColumn(table, SWT.CENTER).setText("mapping");
		...
		editors[2] = new ComboBoxCellEditor(table,TYPES,SWT.READ_ONLY);
		...
		tv.addSelectionChangedListener(new ISelectionChangedListener(){
			public void selectionChanged(SelectionChangedEvent event) {
				FieldsListEditor.this.selectionChanged();
			}
	...
	class PropertyLabelProvider implements ITableLabelProvider {
		...
		public String getColumnText(Object element, int columnIndex) {
			...
			case 1:
				return obj.get("mapping").toString();
			case 2:
				return TYPES[ ((Integer)obj.get("type")).intValue() ];
			}
			return null;
		}
	...
	class PropertyCellModifier implements ICellModifier {
		...
		public void modify(Object element, String property, Object value) {
			...
			if ("name".equals(property))
				...
				p.put("mapping", ((String) value));
			else if ("type".equals(property)){
				p.put("type", ((Integer) value));
			}
			viewer.refresh();
		}
	}
	...
	public void setList(List list){
		...
		for (Iterator iter = list.iterator(); iter.hasNext();) {
			ExtDataField f = (ExtDataField) iter.next();
			Map map = new HashMap();
			map.put("name", f.getName());
			map.put("mapping",f.getMapping());
			int index = ArrayUtils.indexOf(TYPES, f.getType());
			if (index<0){
				index=0;
			}
			map.put("type", new Integer(index));
			properties.add(map);
		}
		tv.refresh();
	}
	...
	public List getList(){
		...
		for (Iterator iter = properties.iterator(); iter.hasNext();) {
			...
			String type = TYPES[ ((Integer)map.get("type")).intValue() ];
			list.add(new ExtDataField(name,mapping,type));
		}
		return list;
	}
